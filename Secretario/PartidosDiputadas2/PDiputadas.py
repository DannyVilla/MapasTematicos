...
Distritos = folium.GeoJson(
    ...
    tooltip=folium.features.GeoJsonTooltip(fields=['nom_dis'], aliases=['DTTO:']),
).add_to(m3)
#print(distrito_local)
###Capa2013
...
def acciones(df_in,anio):
    for row in df_in.itertuples():
        contenido = content_directorio(str(row.NOMBRE), str(row.PARTIDO), str(row.ANIO), str(row.ENTIDAD), str(row.FOTO),anio,str(row.nom_dis))
        popup = folium.Popup(html=contenido, max_width='800')


        if row.ENTIDAD=="Rep. Proporcional":
            if row.PARTIDO == 'PRI':
                marcador_partido='images/PRI_marcador.png'
            elif row.PARTIDO == 'MC':
                marcador_partido="images/MC_marcador.png"

            elif row.PARTIDO == 'PAN':
                marcador_partido="images/PAN_marcador.png"

            elif row.PARTIDO == 'MORENA':
                marcador_partido="images/MORENA_marcador.png"

            elif row.PARTIDO == 'PT':
                marcador_partido="images/PT_marcador.png"

            elif row.PARTIDO == 'PRD':
                marcador_partido="images/PRD_marcador.png"

            elif row.PARTIDO == 'PVEM':
                marcador_partido="images/PVEM_marcador.png"

            elif row.PARTIDO == 'PES':
                marcador_partido="images/PES_marcador.png"
        else:
            marcador_partido = "images/diputadas_marcador.png"

        icon_Dependencia = folium.features.CustomIcon(marcador_partido, icon_size=(70, 70),
                                                          icon_anchor=(22, 59),
                                                          popup_anchor=(3, -54))

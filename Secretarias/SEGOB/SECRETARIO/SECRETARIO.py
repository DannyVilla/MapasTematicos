...
df_secretario_2021 = pd.read_excel("SECRETARIO_OK.xlsx", sheet_name='SECRETARIO_2021')
df_secretario_2022 = pd.read_excel("SECRETARIO_OK.xlsx", sheet_name='SECRETARIO_2022')
df_secretario_2023 = pd.read_excel("SECRETARIO_OK.xlsx", sheet_name='SECRETARIO_2023')

df_secretario_2021 = pd.merge(df_secretario_2021,df_Localidades, on='CVEGEO')
df_secretario_2022 = pd.merge(df_secretario_2022, df_Localidades, on="CVEGEO")
df_secretario_2023 = pd.merge(df_secretario_2023, df_Localidades, on="CVEGEO")

df_secretario_2021['FECHA'] = df_secretario_2021['FECHA'].dt.strftime('%d/%m/%Y')
df_secretario_2022['FECHA'] = df_secretario_2022['FECHA'].dt.strftime('%d/%m/%Y')
df_secretario_2023['FECHA'] = df_secretario_2023['FECHA'].dt.strftime('%d/%m/%Y')
...
layer_Promocion=FeatureGroup(name='Promociones Turisticas (2021 - 2022)',show=False)
...
mc_Seguridad=MarkerCluster()
mc_Mesas_2022=MarkerCluster()

def genera_actividades(df_in):
    for row in df_in.itertuples():
        contenido = content_secretario(str(row.MUNICIPIO), str(row.TEXTO), str(row.FECHA),
                                            str(row.CLASIFICACION),str(row.FOTO),str(row.PUB))

        popup = folium.Popup(html=contenido, max_width='290')
        icon_Dependencia = folium.features.CustomIcon('images/SECRETARIO_marcador.png', icon_size=(60, 60),
                                                      icon_anchor=(22, 59),
                                                      popup_anchor=(3, -54))
...
genera_actividades(df_secretario_2021)
genera_actividades(df_secretario_2022)
...
mc_Seguridad_2023=MarkerCluster()
mc_Mesas_2023=MarkerCluster()

def genera_actividades_2023(df_in):
    for row in df_in.itertuples():
        contenido = content_secretario_2023(str(row.MUNICIPIO), str(row.TEXTO), str(row.FECHA),str(row.URL),
                                            str(row.CLASIFICACION),str(row.FOTO),str(row.PUB))
...
genera_actividades_2023(df_secretario_2023)

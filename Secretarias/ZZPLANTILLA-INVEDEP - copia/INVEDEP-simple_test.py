...
# %%
path_ini = Path("C:/Mapas_COESPO/Resources/")
...
LayerAcciones1 = FeatureGroup(name='Caravanas Culturales', show=True)
LayerAcciones2 = FeatureGroup(name='Brigadas Itinerantes', show=True)
LayerAcciones3 = FeatureGroup(name='Capacitaciones', show=True)
...
for row in df_mapaAccion1.itertuples():
    icon_Dependencia = folium.features.CustomIcon('Marker.png', icon_size=(60, 60), icon_anchor=(22, 59),
                                                  popup_anchor=(3, -54))
    texto_mapa ="<head> <link rel='stylesheet' href='https://use.fontawesome.com/releases/v5.5.0/css/all.css'> <link rel='stylesheet' href='css/estilos.css'></head>"
    texto_mapa = "<div class='overlay' id='overlay'><article><h2>" + row.nom_mun + "</h2><p><b>Municipio: </b>" + row.nom_mun + "<br><b>Acciones: </b>" + str(row.ACCIONES) \
                 ...
                 + "<br><b>Tipo de Peticion más Frecuente: </b>" + str(row.TIPO_DE_PETICION_MAS_FRECUENTE+"</p></div>")
    if row.FOTO != "0":
...
for row in df_mapaAccion3.itertuples():
    ...
    if row.FOTO != "0":
        texto_mapa = texto_mapa + "<img src='" + row.FOTO + "'>"
    popup = folium.Popup(html=texto_mapa, max_width='400')
